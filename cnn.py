...
# genes_indexes = df.columns[df.columns.str.startswith('ENSG')][:900]
# genes = df[genes_indexes]
# labels = df['tumor_stage']
# small_df = pd.concat([genes, labels], axis=1)
# small_df.to_csv('notebooks/data/small_gens_cleaned.csv')
...
#
# Flatten
x_flat = tf.reshape(x_conv1, [-1, 30 * 30 * 32])

# Dense fully connected layer   30 * 30 * 32 --> 256
W_fc1 = weight_variable([30 * 30 * 32, 256])
b_fc1 = bias_variable([1, 256])
...
# Classification layer   256--> 4
W_fc2 = weight_variable([256, 4])
b_fc2 = bias_variable([1, 4])
...
for i in range(n_epochs):
    ...
    Optimizer.run(feed_dict={x: img_genes, y: lbls, keep_prob: .4})
